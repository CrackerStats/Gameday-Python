...
from libmin import game, atbats, hitchart, players, store, CONSTANTS, Fetcher
...
def usage():
	print argv[0], \
		'--year=XXXX', \
		'\n\n', \
		'optional: --month=x,y --day=x,y --type=[aaa, win] --delta --verbose'
	raise SystemExit
...
if __name__ == '__main__':
	...
	for month in months:
		...
		DB.query2(sql)
		DB.save()
		DB.query2("update atbat set b_height = 12*SUBSTRING(b_height,1,1)+SUBSTRING(b_height,3,2) where b_height like '%-%'")
		DB.query2("update player set height = 12*SUBSTRING(height,1,1)+SUBSTRING(height,3,2) where height like '%-%'")
